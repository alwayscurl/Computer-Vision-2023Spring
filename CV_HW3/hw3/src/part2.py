...
def planarAR(REF_IMAGE_PATH, VIDEO_PATH):
    ...
    while (video.isOpened()):
        ...
        if ret:  ## check whethere the frame is legal, i.e., there still exists a frame
            ...
            aruco_corners, _, _ = aruco.detectMarkers(frame, arucoDict, parameters=arucoParameters)
            # TODO: 2.find homograpy
            # function call to solve_homography()
            H = solve_homography(ref_corns, aruco_corners[0][0])
            # TODO: 3.apply backward warp
            # function call to warping()
            xmin = min(aruco_corners[0][0][0][0], aruco_corners[0][0][1][0], aruco_corners[0][0][2][0], aruco_corners[0][0][3][0])
            xmax = max(aruco_corners[0][0][0][0], aruco_corners[0][0][1][0], aruco_corners[0][0][2][0], aruco_corners[0][0][3][0])
            ymin = min(aruco_corners[0][0][0][1], aruco_corners[0][0][1][1], aruco_corners[0][0][2][1], aruco_corners[0][0][3][1])
            ymax = max(aruco_corners[0][0][0][1], aruco_corners[0][0][1][1], aruco_corners[0][0][2][1], aruco_corners[0][0][3][1])
