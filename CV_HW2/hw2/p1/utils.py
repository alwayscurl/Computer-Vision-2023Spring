...
def get_tiny_images(img_paths):
    ...
    for i in range(len(img_paths)):
        ...
        img = img.crop(((width - crop_size) // 2, (height - crop_size) // 2, (width + crop_size) // 2, (height + crop_size) // 2))
...
def build_vocabulary(img_paths, vocab_size=400):
    ...
    for i in range(len(img_paths)):
        print(i)
        img = np.asarray(Image.open(img_paths[i]), dtype='float32')
        _, feat = dsift(img, step = [3, 3], fast = True)
        # print(feat.shape)
        sift_features.append(feat)
        # print(sift_features.shape)
    sift_features = np.concatenate(sift_features, axis=0).astype('float32')
    print("start kmeans")
    vocab = kmeans(sift_features, num_centers=vocab_size)
    ##################################################################################
    #                                END OF YOUR CODE                                #
    ##################################################################################
...
def get_bags_of_sifts(img_paths, vocab):
    ...
    for i in range(len(img_paths)):
        print(i)
        img = np.asarray(Image.open(img_paths[i]))
        _, sift_features = dsift(img, step = [3, 3], fast = True)
        distance = cdist(sift_features, vocab, metric='euclidean')
        hist= np.histogram(np.argmin(distance, axis=1), bins=np.arange(len(vocab)+1))[0] 
        hist = hist / len(sift_features)
...
def nearest_neighbor_classify(train_img_feats, train_labels, test_img_feats):
    ...
    for i in range(len(test_img_feats)):
        k_nearest_indices = np.argsort(distances[i])[:k]  # Indices of k-nearest training features
        k_nearest_labels = [train_labels[idx] for idx in k_nearest_indices]  # Labels of k-nearest training features
        # Vote for the final label id
        label_votes = [CAT2ID[label] for label in k_nearest_labels]
        final_label_id = np.bincount(label_votes).argmax()  # Most common label id
        final_label = CAT[final_label_id]  # Convert label id back to string
        test_predicts.append(final_label)
    # distance = cdist(test_img_feats, train_img_feats, metric='minkowski', p=0.5)
    # for i in range(len(test_img_feats)):
    #     k_nearest_feature_id = np.argsort(distance[i])[:k]
    #     count = np.zeros(15)
    #     for j in range(k):
    #         count[CAT2ID[train_labels[k_nearest_feature_id[j]]]] += 1
    #     test_predicts.append(CAT[np.argmax(count)])
